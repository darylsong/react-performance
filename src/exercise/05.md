...
In a context provider component, it may be useful to memoize the props that are
being passed to the child components so as to avoid unnecessary renders of the
child components. In the example below, the context provider component
`AppProvider` passes the props `value` to the `AppStateContext` context. The
`value` props is an array comprising `state` and `dispatch` and is created in
the body of the `AppProvider` component. As a result, a new `value` props is
created each time the `AppProvider` component is re-rendered, even if `state`
or `dispatch` do not change. This causes the child components consuming the
`AppStateContext` context to be re-rendered, even if these components have been
memoized using `React.memo`. To eliminate these unnecessary re-renders, we can
memoize the `value` props using the `useMemo` hook instead.

However, the problem with the above solution is that a child component that
requires **only** `dispatch` will have to consume the `AppStateContext` which
provides the `value` props which comprises both `state` and `dispatch`. As a
result, the component that requires **only** `dispatch` will be unnecessarily
re-rendered in the event that `state` changes. This can be avoided by using
different contexts for both `state` and `dispatch`. The component that only
requires `dispatch` will then only consume the context providing `dispatch`,
and will thus not be re-rendered when `state` changes.
