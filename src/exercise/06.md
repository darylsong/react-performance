...
To improve performance, we should try to separate our app's state logically into
domain-specific context providers. This is to ensure that only the necessary
context consumers are updated on state changes.

A more advanced pattern for improving performance that is used in the second
extra credit exercise here is the use of a memoized middle-man component. Before,
each `Cell` will obtain the state of the grid and, thereafter, display the state
of the particular cell. Thus, whenever the state of the grid is changed, each
`Cell` component will be re-rendered. Each `Cell` component will again obtain the
state of the grid and then display the state of the cell.

However, to improve performance, we can create a new component `CellImpl` that
renders the cell state. This `CellImpl` will not access the state of the grid
directly. Instead, the `Cell` component will serve as a "iddle-man" to access
the state of the grid, and then pass the `cell` state to the `CellImpl`
component to render. We can then memoize the `CellImpl` component using
`React.memo` and, thus, the component will only re-render when `cell` changes.
This modification still results in `Cell` being re-rendered when the state of
the grid changes. However, performance is improved as `CellImpl` will not re-
render if `cell` does not change.

To improve readability and re-usability of the above code, we can also make a
generic higher order component (here, `withStateSlice`) to produce the same
result.

**Come back to the recoil implementation section.**
