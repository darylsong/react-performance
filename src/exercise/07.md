...
The `Profiler` API allows one to measure the cost of rendering a particular
React tree. This can be done by wrapping the React tree with <Profile> and by
passing it an `onRender` callback function. React will call the `onRender`
