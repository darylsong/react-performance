...
Take note of the difference between "render" and "commit". "Render" simply
means the creation of React elements. On the other hand, "commit" means to
update the DOM. Note that just because a component is re-rendered (e.g.,
a child component re-renders because its parent re-renders) does not mean
that the DOM that represents the child component is also re-rendered. Given
that the updating of the DOM is usually the process that is slow, a 
re-render may not necessarily be slow (in the event that the DOM is not
updated). Thus, the focus should usually be on reducing the render time
rather than on reducing the amount of re-renders.

The `React.memo` function can be used to reduce the number of times a React
component is re-rendered. The function memoizes a particular React component,
and this component will only re-render if the props that are passed to the
component are changed. Note that React uses a shallow comparison to determine
whether the props have been changed.

Note also that the `React.memo` function takes in a second argument which is
a comparison function which will be used to determine whether the props passed
to the component have been changed. The comparison function should have the
following API:
...
The comparison function should return `true` if the props should be treated as
equal and `false` otherwise. In the event that comparison returns a `false`
value, React will then cause the component to re-render, as the props passed
to the component has changed.

However, try to avoid using the above, if possible. For example, where possible,
try to pass primitive values to a component instead of complex values. This will
then allow us to rely on React's default comparison function.
