...
We can use the `useMemo` hook to reduce the time required to render a React
component by memoizing values that are expensive to compute. In the exercise
below, the user can search from a list of cities. The user enters a search
term into an `input` and the `getItems` function is called on this input.
The `getItems` function is an expensive calculation. Thus, we can improve
performance by memoizing the value of `getItems` function call.

A more advanced performance enhancement is to use web works.
**Come back to this.**
